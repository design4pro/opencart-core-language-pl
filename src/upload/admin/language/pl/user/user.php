<?php
// Heading
$_['heading_title'] = 'Użytkownicy';

// Text
$_['text_success'] = 'Sukces: Zmodyfikowano użytkowników!';
$_['text_list'] = 'Lista użytkowników';
$_['text_add'] = 'Dodaj użytkownika';
$_['text_edit'] = 'Edytuj użytkownika';

// Column
$_['column_username'] = 'Nazwa użytkownika';
$_['column_status'] = 'Status';
$_['column_date_added'] = 'Data dodania';
$_['column_action'] = 'Akcje';

// Entry
$_['entry_username'] = 'Nazwa użytkownika';
$_['entry_user_group'] = 'Grupa użytkownika';
$_['entry_password'] = 'Hasło';
$_['entry_confirm'] = 'Potwierdź hasło';
$_['entry_firstname'] = 'Imię';
$_['entry_lastname'] = 'Nazwisko';
$_['entry_email'] = 'Adres poczty elektronicznej';
$_['entry_image'] = 'Obrazek';
$_['entry_status'] = 'Status';

// Error
$_['error_permission'] = 'Ostrzeżenie: Nie masz uprawnień do modyfikowania użytkowników!';
$_['error_account'] = 'Ostrzeżenie: Nie możesz usunąć własnego konta!';
if ('2.3.0.0' > VERSION) {
  $_['error_exists'] = 'Ostrzeżenie: Nazwa użytkownika została już wykorzystana!';
}
$_['error_exists_username'] = 'Ostrzeżenie: Nazwa użytkownika została już wykorzystana!';
$_['error_username'] = 'Nazwa użytkownika musi zawierać od 3 do 20 znaków!';
$_['error_password'] = 'Hasło musi zawierać od 4 do 20 znaków!';
$_['error_confirm'] = 'Hasła różnią się od siebie!';
$_['error_firstname'] = 'Imię musi zawierać od 1 do 32 znaków!';
$_['error_lastname'] = 'Nazwisko musi zawierać od 1 do 32 znaków!';
$_['error_email'] = 'Adres poczty elektronicznej wygląda na nieprawidłowy!';
$_['error_exists_email'] = 'Ostrzeżenie: Adres poczty elektronicznej jest już zarejestrowany!';
